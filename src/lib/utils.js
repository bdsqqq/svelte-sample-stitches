	...
	for (let r = 0; r <= dim_size - 1; r++) {
		for (let c = 0; c <= dim_size - 1; c++) {
			if (
				...
				board[r][c] = board[r][c].replace(' ', getNumOfNeighboringBombs(board, r, c));
			}
		}
	}
	...
	for (let r = 0; r <= dim_size - 1; r++) {
		for (let c = 0; c <= dim_size - 1; c++) {
			if (
				...
				board[r][c] = board[r][c] = getNumOfNeighboringBombsIfBombsAre9(board, r, c);
			}
		}
	}
...
export const getNumOfNeighboringBombsIfBombsAre9 = (board, row, col) => {
	let bombNum = 0;
